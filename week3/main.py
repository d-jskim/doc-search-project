    ...
    eval_dataset = [
        {
            ...
            "relevant_doc_ids": ["D030", "D031", "D055"]  # 3개
        },
        {
            "query": "python decorators explained",
            "relevant_doc_ids": ["D010", "D051"]  # 2개
        },
        {
            "query": "list comprehension",
            "relevant_doc_ids": ["D001", "D059"]  # 2개
        },
        {
            "query": "python lambda functions",
            "relevant_doc_ids": ["D009", "D003"]  # 2개
        },
        {
            "query": "python exception handling",
            "relevant_doc_ids": ["D005"]  # 1개
        },
        {
            "query": "git merge conflicts",
            "relevant_doc_ids": ["D018", "D014"]  # 2개
        },
        {
            "query": "git merge vs rebase",
            "relevant_doc_ids": ["D014", "D015", "D013"]  # 3개
        },
        {
            "query": "gradient descent",
            "relevant_doc_ids": ["D023", "D024"]  # 2개
        },
        {
            "query": "dropout regularization",
            "relevant_doc_ids": ["D056", "D055", "D031"]  # 3개
        },
        {
            "query": "handling missing data in pandas",
            "relevant_doc_ids": ["D043", "D049"]  # 2개
        },
        {
            "query": "numpy indexing",
            "relevant_doc_ids": ["D033", "D039", "D057"]  # 3개
        },
        {
            "query": "cnn image classification",
            "relevant_doc_ids": ["D060", "D031"]  # 2개
        },
        {
        ...
        },
        {
            ...
            "relevant_doc_ids": ["D030"] # backpropagation이라는 핵심 단어가 없음
            # D030 Backpropagation Algorithm
        },
        {
            "query": "how to recover from merge mistakes",
            "relevant_doc_ids": ["D018"] # conflicts라는 단어가 없어 교집합이 줄어듦
            # D018 Git Resolving Merge Conflicts
        },
        {
            "query": "remove null values from dataframe",
            "relevant_doc_ids": ["D043", "D049"] # null values ↔ missing data 표현 차이 때문에 Baseline이 불리
            # D043 Handling Missing Data in pandas
            # D049 pandas Data Cleaning Techniques
        }
    ]
    return eval_dataset


def precision_at_k(search_id_lst, answer_id_lst, top_k):

    search_id_lst = search_id_lst[:top_k] # 검색 함수에서 입력받은 top_k로 리턴하지만, 문제 요구사항대로 구현 / 재할당

    intersection = set(search_id_lst) & set(answer_id_lst)
    precision = len(intersection) / len(search_id_lst)

    return precision

def reciprocal_rank(search_id_lst, answer_id_lst):

    for rank, doc_id in enumerate(search_id_lst, start=1):
        if doc_id in answer_id_lst:
            reciprocal_rank_val = 1 / rank
            return reciprocal_rank_val

    return 0.0

def run_evaluation(eval_dataset, search_func, k):
    ...
    for data in eval_dataset:
        query = data["query"]
        answer_lst = data["relevant_doc_ids"]
        search_lst = search_func(query)["doc_id"].tolist()[:k]

        # precision 계산
        precision = precision_at_k(search_lst, answer_lst, k)
        precision_lst.append(precision)
        # reciprocal_rank 계산
        reciprocal_rk = reciprocal_rank(search_lst, answer_lst)
        reciprocal_rank_lst.append(reciprocal_rk)


        # print(f"QUERY: {query}")
        # print(f"ANSWERS: {answer_lst}")
        # print(f"SEARCH RESULTS: {search_lst}")
        # print(f"precision: {precision}")
        # print(f"reciprocal rank: {reciprocal_rk}")

        mean_precision = np.mean(precision_lst)
        mean_MMR = np.mean(reciprocal_rank_lst)


        return {
                f"Precision@{k}": round(mean_precision, 4),
                "MRR": round(mean_MMR, 4)
                }

def analyze_failures(eval_dataset, search_func, k):

    print(f"=== 실패 케이스 (Top-{k} 안에 정답 없음) ===")
    ...
    for data in eval_dataset:
        ...
        reciprocal_rk = reciprocal_rank(search_lst, answer_lst)
        # 검색 결과가 정답에 없을 경우, 출력
        if reciprocal_rk == 0.0:
            print(f"질문: {query}")
            print(f"  정답 doc_id: {answer_lst}")
            print(f"  검색 결과   : {search_lst}")
...
def main():
    ...
    # 성능 평가
    baseline_search_func = lambda q: keyword_search(q, df, top_k)
    tfidf_search_func = lambda q: tfidf_search(q, df, tfidf_matrix, tfidf_vectorizer, top_k)

    baseline_result = run_evaluation(eval_dataset, baseline_search_func, top_k)
    tfidf_result = run_evaluation(eval_dataset, tfidf_search_func, top_k)

    comparison_df = pd.DataFrame(
        [baseline_result, tfidf_result],
    ...
    # 성능 분석(실패)
    print("=== TF-IDF Search 실패 분석 ===")
    analyze_failures(eval_dataset, tfidf_search_func, top_k)

    print("=== Baseline Search 실패 분석 ===")
    analyze_failures(eval_dataset, baseline_search_func, top_k)
